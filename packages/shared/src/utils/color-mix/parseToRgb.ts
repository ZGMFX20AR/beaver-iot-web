...
import nameToHex from './helpers/nameToHex';
 ...
 *
 * @example
 * 1. Assigns `{ red: 255, green: 0, blue: 0 }` to color1
 * const color1 = parseToRgb('rgb(255, 0, 0)');
 * 2. Assigns `{ red: 92, green: 102, blue: 112, alpha: 0.75 }` to color2
 * const color2 = parseToRgb('hsla(210%, 40%, 0.75)');
 */
export default function parseToRgb(color: string): RgbColor | RgbaColor | undefined {
    ...
    if (rgbMatched) {
        return {
            red: parseInt(`${rgbMatched[1]}`),
            green: parseInt(`${rgbMatched[2]}`),
            blue: parseInt(`${rgbMatched[3]}`),
        };
    }
    const rgbaMatched = rgbaRegex.exec(normalizedColor.substring(0, 50));
    if (rgbaMatched) {
        return {
            red: parseInt(`${rgbaMatched[1]}`),
            green: parseInt(`${rgbaMatched[2]}`),
            blue: parseInt(`${rgbaMatched[3]}`),
            alpha:
    ...
    if (hslMatched) {
        const hue = parseInt(`${hslMatched[1]}`);
        const saturation = parseInt(`${hslMatched[2]}`) / 100;
        const lightness = parseInt(`${hslMatched[3]}`) / 100;
        ...
        return {
            red: parseInt(`${hslRgbMatched[1]}`),
            green: parseInt(`${hslRgbMatched[2]}`),
            blue: parseInt(`${hslRgbMatched[3]}`),
        };
    }
    const hslaMatched = hslaRegex.exec(normalizedColor.substring(0, 50));
    if (hslaMatched) {
        const hue = parseInt(`${hslaMatched[1]}`);
        const saturation = parseInt(`${hslaMatched[2]}`) / 100;
        const lightness = parseInt(`${hslaMatched[3]}`) / 100;
        const rgbColorString = `rgb(${hslToRgb(hue, saturation, lightness)})`;
        const hslRgbMatched = rgbRegex.exec(rgbColorString);
        if (!hslRgbMatched) {
            return;
